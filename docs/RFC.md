...
After company confirmation, the user selects annual or quarterly, then the number of periods. The period count options are capped to whatever yfinance actually returns for that company — no hardcoded spec max. `YahooClient.get_available_periods(period)` does a lightweight fetch (one statement, column count) before the prompt is shown. All 7 sheets are always exported — the user makes no sheet selection. Sheet 7 is included automatically if `OPENAI_API_KEY` is present, skipped silently if not. The Yahoo Finance client always fetches all 6 data types via a single `fetch_all()` call.
